...
router.post("/", authMiddleware, async (req, res) => {
  ...
  try {
    const [result]: any = await pool.query(`
      INSERT INTO cours (id_coach, type_cours, date_cours, heure_debut, duree_minutes, salle, capacite_max, places_restantes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `, [id_coach, type_cours, date_cours, heure_debut, duree_minutes || 60, salle, capacite_max, capacite_max]);
